...
object CanvasUtils {
     ...
     * *曲线不从点穿过
     */
    fun getLineGraph2(path: Path?, points: List<PointF>?): Path {
        var mPath = path ?: Path()
        if (!points.isNullOrEmpty()) {
            ...
            val p_anchor = PointF(0F, 0F) //锚点
            val p_current = PointF(0F, 0F)
            val p_end = PointF(0F, 0F) //曲线终点

            while (index < size) {
                p_current.set(points[index])

                if (index == 0) {
                    mPath.moveTo(p_current.x, p_current.y)
                } else {

                    p_end.set((p_current.x + p_anchor.x) / 2f, (p_current.y + p_anchor.y) / 2f)
                    mPath.quadTo(p_anchor.x, p_anchor.y, p_end.x, p_end.y)

                    if (index == size - 1) {
                        mPath.lineTo(p_current.x, p_current.y)
                    }
                }
                p_anchor.set(p_current)
                index++
            }
        }
        return mPath
     ...
     * *曲线从点中穿过
     */
    fun getLineGraph3(path: Path?, points: List<PointF>?): Path {
        var mPath = path ?: Path()
        if (!points.isNullOrEmpty()) {
            mPath.reset()
            ...
            while (index < size) {
                ...
                if (index == 0) {
                    ...
                    val y_first = p_end.y //上一个Y点

                    p_end.set((p_current.x + p_anchor.x) / 2f, (p_current.y + p_anchor.y) / 2f)
                    val y_contrl = (p_anchor.y - (0.25F * y_first + 0.25F * p_end.y)) * 2F //反推控制点的Y值
                    mPath.quadTo(p_anchor.x, y_contrl, p_end.x, p_end.y)
    ...
    fun getLineGraph4(path: Path?, points: List<PointF>?): Path {

        val array = getPointfArray(points)
        return getLineGraph2(path, array)
    }

    fun getPointfArray(points: List<PointF>?): List<PointF> {
        ...
        points?.forEachIndexed { index, current ->
            if (index == 0 || index == (size - 1)) {
                ...
                val p_first = points[index - 1]
                val p_last = points[index + 1]
                val start = PointF((p_first.x + current.x) * 0.5f, (p_first.y + current.y) * 0.5f)
                val end = PointF((p_last.x + current.x) * 0.5f, (p_last.y + current.y) * 0.5f)
                array.add(getControlPointF(start, end, current))
            }
        }
        return array
    }
